...
def format_external_liquidations_for_table(liquidations: List, symbol_mapping: Dict[str, str] = None) -> List[Dict[str, Any]]:
    ...
    for liq in liquidations:
        ...
        row = {
            ...
            "Pre-Liq Twyne LTV (%)": f"{twyne_pre_ltv*100:.2f}",
            "Pre-Liq Euler LTV (%)": f"{euler_pre_ltv*100:.2f}",
            "External LLTV (%)": f"{euler_liq_ltv*100:.2f}",
            "Twyne LLTV (%)": f"{twyne_liq_ltv*100:.2f}",
            "External LTV Exceedance (%)": f"{external_ltv_exceedance*100:.2f}",
            "Twyne LTV Exceedance (%)": f"{internal_ltv_exceedance*100:.2f}",
            "Closing Factor (%)": f"{closing_factor*100:.2f}",
            "Liquidation Incentive (%)": f"{liquidation_incentive*100:.2f}",
            "CLP Loss (%)": f"{clp_loss*100:.2f}",
